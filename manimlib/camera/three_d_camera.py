...
class ThreeDCamera(Camera):
    """使用球坐标系，``phi,theta`` 和球坐标系中意义相同。
    ``distance`` 表示画面扭曲程度，不表示相机距离。
    ``gamma`` 表示相机转角。
    ``frame_center`` 表示相机的中心，为一个 ``ndarray``。
    ``should_apply_shading`` 是一个bool值，为True时根据光源位置设置光泽。
    ``light_source_start_point`` 为一个 ``ndarray`` 表示光源位置
    """
    CONFIG = {
        "shading_factor": 0.2,
        "distance": 20.0,
