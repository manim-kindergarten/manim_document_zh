...
:class:`~mobject.svg.svg_mobject.SVGMobject` 用于解析输入的SVG文件，并生成一个
:class:`~mobject.types.vectorized_mobject.VMobject`。由于其方法都比较内部，
主要是根据SVG的元素来生成点集，这里就不列出文档字符串了。

MK做了一个关于常用 :class:`~mobject.svg.svg_mobject.SVGMobject` 的的视频：
