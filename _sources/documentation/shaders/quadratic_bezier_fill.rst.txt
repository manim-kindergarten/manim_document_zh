...
那么就不使用直接三角形绘制。
    ...
    假设有一个圆，它由八段贝塞尔曲线拼接而成，每段由 3 个控制点构成。给这个圆上填充色，我们对它进行三角剖分，
    分割为一个正八边形和边上的八个弓形。而后，我们再对八边形进行剖分，将它割成一些三角形。最终得到的一系列“图元”，
    包括三角形和弓形，它们都是由 3 个控制点构成的。

    .. image:: https://cdn.jsdelivr.net/gh/manim-kindergarten/CDN@master/manimgl_assets/shaders/circle_triangulation.png

    对于内部的这些三角形，注意到它们的三个顶点几乎都是不相邻的整数，我们可以直接采用直接三角形方案来上色。

    而对于外面的这些弓形，它们的三个控制点几乎都是相邻的整数，因此我们不采用 ``fill_all`` 的方案，
    而是而采用贝塞尔曲线的上色方案，同时对于这些边缘的图形，需要加一些抗锯齿以优化视觉效果。
...
而对于边缘上的弓形，我们就需要使用 sdf 符号距离函数来进行一些处理。我们采用这样的方案：
    ...
    我们都按照三角图元来上色，这样也就直接覆盖了上面的情况，即内部三角图元的上色方法。

    而边缘的弓形此时也被涂成了三角形，此时 ``fill_all`` 参数的用途就得以体现了：当 ``fill_all == 1.0``
    说明此时是直接三角形绘制；否则，我们就按照贝塞尔曲线的上色，通过 sdf 计算，在弓形内部的像素点，保持其透明度不变；
    而在弓形外部的像素点，其透明度为 0 
